...
{
    aspect = mc->width_phys / (float) mc->height_phys;
    ...
    else
    {
        ...
        const float stuff = 0.5f;
        float fovRad = fov * (PI / 180.0f);
        float fovFull = (180.0f / PI) * 2.0f * atanf(tanf(fovRad * 0.5f) * (aspect / baseAspect));
        fov = fov + (fovFull - fov) * stuff;
    }
}
...
void GameRenderer::setupGuiScreen(int forceScale /*=-1*/)
{
    ...
    glViewport(0, 0, mc->width, mc->height);  // ADD THIS LINE
